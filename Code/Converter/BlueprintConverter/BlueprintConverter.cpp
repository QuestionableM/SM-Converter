...
BlueprintConv::AddObjectFunction BlueprintConv::GetAddObjectFunction()
{
	switch (BlueprintConverterSettings::SeparationType)
	{
	case BPObjectSep_All: return BlueprintConv::BlueprintAddObject_SeparateAll;
	case BPObjectSep_Joints:
		break;
	case BPObjectSep_Uuid:
		break;
	case BPObjectSep_Color:
		break;
	case BPObjectSep_UuidAndColor:
		break;
	}
...
{
	switch (v_entity->Type())
	{
	case EntityType::Block:
		DebugOutL("Block: ", v_entity->GetUuid().ToString());
		break;
	case EntityType::Joint:
		DebugOutL("Joint: ", v_entity->GetUuid().ToString());
		break;
	case EntityType::Part:
		DebugOutL("Part: ", v_entity->GetUuid().ToString());
		break;
	}
...
void BlueprintConv::ConvertToModel(const std::wstring& bp_path, const std::wstring& bp_name, ConvertError& v_error)
{
	...
	BlueprintConv::AddObjectFunction v_add_obj_func = BlueprintConv::GetAddObjectFunction();

	Blueprint* v_blueprint = Blueprint::FromFileErrorChecked(bp_path, v_add_obj_func, v_error);
	if (v_blueprint)
	{
